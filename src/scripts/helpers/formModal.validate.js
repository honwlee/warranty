define([
    ...
    "text!./_partials.hbs",
    "handlebars"
], function($, server, skylarkjs, validates, partial, itemActions, toastr, partialsTpl, handlebars) {
                ...
                if (xhr.status === 200) {
                    ...
                    if (result.status) {
                        ...
                        if (result.auth) {
                            ...
                            toastr.error("数据已存在：(" + validates[result.key].nameMsg + ":" + result.value + ")");
                            selector.find(".override-btn").removeClass("hide").off("click").on("click", function() {
                                opt.override = true;
                                save(name, selector, opt, callback);
                            });
                        } else {
    ...
    return {
        ...
        show: function(type, data, callback, prepareData) {
            ...
            modal.find(".override-btn").addClass("hide");
            modal.modal('show');
